...
//      [button setTitle:buttonTexts[i] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"[face%@]",buttonTexts[i]]] forState:UIControlStateNormal];
        button.tag = [buttonTexts[i] integerValue];
...
- (void)emojiButtonPressed:(UIButton *)button {
  ...
  [self.delegate emojiPageView:self didUseEmoji:[NSString stringWithFormat:@"[face%02d]",button.tag]];
}
